...
def init_db():
    ...
    # bump Plex's own addedAt — can't resurface a title already recorded here.
    # Keyed by provider id (like assignments_by_provider) so rows survive Plex
    # rating-key changes; items with no provider id fall back to a RatingKey key.
