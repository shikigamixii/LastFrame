...
def test_norm_created_handles_z_and_long_fractions():
    out = ra._norm_created("2024-06-01T12:34:56.1234567Z")
    assert out is not None and out.endswith("+00:00") and out.startswith("2024-06-01T12:34:56")
    assert ra._norm_created("") is None
    assert ra._norm_created("not-a-date") is None


# ── _monitored_sections (Jellyfin library shape) ──────────────────────
def test_monitored_sections_respects_scope():
    sections = [{"Id": "1", "CollectionType": "movies"},
                {"Id": "2", "CollectionType": "tvshows"},
                {"Id": "3", "CollectionType": "music"}]
    assert ra._monitored_sections({"show_all_libraries": True}, sections) == [("1", "movies"), ("2", "tvshows")]
    scoped = ra._monitored_sections(
        {"show_all_libraries": False, "monitored_libraries": ["2"]}, sections)
    assert scoped == [("2", "tvshows")]
