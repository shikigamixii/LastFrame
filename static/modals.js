...
async function openSettings(){
  ...
  h+='</div>';
  // Webhook section
  h+='<div style="margin-top:1.5rem;border-top:1px solid var(--border);padding-top:1rem"><h3 style="margin-bottom:0.5rem">Jellyfin Webhook</h3>';
  const whUrl=window.location.origin+"/api/webhook/jellyfin";
  h+='<div style="font-size:0.78rem;color:var(--text-muted);margin-bottom:0.5rem">Add this URL in the Jellyfin Webhook Plugin settings (Dashboard → Webhooks):</div>';
  ...
  h+='<div style="font-size:0.78rem;color:var(--text-muted);margin-bottom:0.75rem">Import past watch events from Jellyfin\'s play history for all users. Safe to run multiple times — existing records are not overwritten.</div>';
  h+='<button class="btn-settings" id="backfillBtn" onclick="importJellyfinHistory()">Import Jellyfin History</button>';
...
async function importJellyfinHistory(){
  const btn=document.getElementById("backfillBtn");
  const status=document.getElementById("backfillStatus");
  if(!btn||!status)return;
  btn.disabled=true;btn.textContent="Importing…";
  status.style.color="var(--text-muted)";status.textContent="Fetching history from Jellyfin, this may take a minute…";
  try{
    ...
    btn.disabled=false;btn.textContent="Import Jellyfin History";
  }
}
