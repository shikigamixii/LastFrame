...
These functions need no Plex server or DB connection, so they're cheap to test
and exactly the spots where a regression would silently mis-count watches or
delete the wrong media.
"""
import bcrypt

import config_store
from plex_api import parse_plex_guids
from webhook_state import is_item_watched_pwe, get_last_played_pwe
from assignments import _resolve_target


# ── parse_plex_guids ──────────────────────────────────────────────────
def test_parse_plex_guids_basic():
    guids = [{"id": "tmdb://123"}, {"id": "imdb://tt9"}, {"id": "tvdb://77"}]
    assert parse_plex_guids(guids) == {"Tmdb": "123", "Imdb": "tt9", "Tvdb": "77"}


def test_parse_plex_guids_ignores_unknown_and_empty():
    assert parse_plex_guids([]) == {}
    assert parse_plex_guids([{"id": "plex://abc"}, {"id": ""}, {}]) == {}
